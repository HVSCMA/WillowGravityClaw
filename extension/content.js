...
function updateSidecarUrl(leadId) {
    ...
    if (iframe && leadId) {
        const newUrl = `https://gravity-claw-production-6f74.up.railway.app/?fublead=${leadId}&mode=sidecar`;
...
async function harvestSensoryData() {
    ...
    try {
        await fetch("https://gravity-claw-production-6f74.up.railway.app/api/sensory/fub_lead", {
            method: "POST",
